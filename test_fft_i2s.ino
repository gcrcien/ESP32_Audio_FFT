...
int waterfall_top = 121; // Y donde empieza el waterfall

// ==== FFT View Settings ====
bool useNoiseSubtraction = false;

int fft_y_bottom = 120;
int fft_height = 80;
...
void generateColorLUT() {
  for (int intensity = 0; intensity < 256; intensity++) {
    ...
    if (intensity < 64) {
      ...
      r = 4 * (intensity - 128);
      g = 255;
      b = 255 - 4 * (intensity - 128);
    } else {
      r = 255;
      g = 255 - 4 * (intensity - 192);
      b = 0;
    }
...
void drawFFTLine(double *magnitudes, int y_bottom, int height1) {
  ...
  float scale = (float)height1 / 2800000.0f;
...
void drawWaterfallLine(double* magnitudes) {
  ...
  for (int x = 0; x < bins_to_draw; x++) {
    ...
    int intensity = constrain(map(current, 0, 990000, 0, 255), 0, 255);
    waterfallBuffer[waterfallIndex][x] = colorLUT[intensity];
  }

  // Dibujar líneas con la más reciente arriba
...
void setupI2S() {
  i2s_config_t i2s_config = {
    ...
    //.pdm_pcm_conv_t = PDM_PCM_CONV_DISABLE,
    .intr_alloc_flags = 0,
    .dma_buf_count = 4,
    .dma_buf_len = 512,
    .use_apll =  true,
    .tx_desc_auto_clear = true,
    .fixed_mclk = 256 * SAMPLING_FREQ
  };
...
for (int j = 0, i = 0; j < NUM_SAMPLES; j++, i += 4) {
  int32_t sample = /*((int32_t)(i2s_data[i + 0]) << 24) |*/
                   ((int32_t)(i2s_data[i + 2]) << 16) |
                   ((int32_t)(i2s_data[i + 3]) << 8);
  sample >>= 8;

  if (sample < minVal) minVal = sample;
  if (sample > maxVal) maxVal = sample;

  vReal[j] = (double)sample;// 8388608.0;
  vImag[j] = 0.0;
}

//Serial.printf("Muestras: min=%ld, max=%ld\n", minVal, maxVal);
}

void setup() {
  setupI2S();
  generateColorLUT();  // <== inicializa la tabla de colores
...
/*
  ...
  for (int k = 0; k < 10; k++) {
    ...
    captureSamples();  // Lee nuevos datos

    // Aplica ventana y FFT
    FFT.windowing(FFTWindow::Hamming, FFTDirection::Forward);
    FFT.compute(vReal, vImag, NUM_SAMPLES, FFT_FORWARD);
    FFT.complexToMagnitude(vReal, vImag, NUM_SAMPLES);
    ...
    for (int i = 0; i < NUM_SAMPLES / 2; i++) {
      float freq = ((float)i * SAMPLING_FREQ) / NUM_SAMPLES;
      Serial.printf("Freq: %7.2f Hz\tMag: %.4f\n", freq, vReal[i]);
    }
